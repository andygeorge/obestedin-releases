...
- Obestedin will read the list of plugins in `community-plugins.json`.
- The `name`, `author` and `description` fields are used for searching.
- When the user opens the detail page of your plugin, Obestedin will pull the `manifest.json` and `README.md` from your GitHub repo).
...
- If your `manifest.json` requires a version of Obestedin that's higher than the running app, your `versions.json` will be consulted to find the latest version of your plugin that is compatible.
- When the user chooses to install your plugin, Obestedin will look for your GitHub releases tagged identically to the version inside `manifest.json`.
- Obestedin will download `manifest.json`, `main.js`, and `styles.css` (if available), and store them in the proper location inside the vault.
